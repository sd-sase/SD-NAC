...
sub _expandMapping {
    ...
    while (my ($attr, $val) = each %$switch) {
        if ($attr =~ /(.*)(AccessList|Vlan|Url|Role|Vpn|Interface)$/) {
            my $type = $2;
            my $role = $1;
    ...
    for my $k (qw(AccessListMapping VlanMapping UrlMapping ControllerRoleMapping VpnMapping InterfaceMapping))  {
...
sub _flattenRoleMappings {
    my ( $switch ) = @_;
    for my $namespace (qw(AccessListMapping VlanMapping UrlMapping ControllerRoleMapping VpnMapping InterfaceMapping))  {
...
sub _deleteRoleMappings {
    my ( $switch ) = @_;
    delete @{$switch}{qw(AccessListMapping VlanMapping UrlMapping ControllerRoleMapping VpnMapping InterfaceMapping)};
}
