...
sub build_child {
    ...
    while ( my ($name, $switch) = each %tmp_cfg) {
        ...
        my %merged = ( Vlan => {}, Role => {}, AccessList => {} , Url => {} , Vpn => {}, Interface => {});
        my %roles;
        foreach my $key ( grep {/(Vlan|Role|AccessList|Url|Vpn|Interface)$/} keys %{$switch} ) {
            next unless my $value = $switch->{$key};
            if ( my ( $type_key, $type ) = ( $key =~ /^(.+)(Vlan|Role|AccessList|Url|Vpn|Interface)$/ ) ) {
